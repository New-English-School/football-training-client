...
export class BaseService<T> {
  ...
  async findAll(params?: Record<string, string | number>): Promise<T[]> {
    const res: AxiosResponse<T[] | PaginatedResponse<T>> = await apiClient.get(
      this.endpoint,
      { params }
    );
  ...
  async update<K extends Partial<T>>(id: number, data: K): Promise<T> {
    const res: AxiosResponse<T> = await apiClient.patch(
      `${this.endpoint}/${id}`,
      data
    );
    return res.data;
  ...
  async remove(id: number): Promise<{ success: boolean }> {
    const res: AxiosResponse<{ success: boolean }> = await apiClient.delete(
      `${this.endpoint}/${id}`
    );
    return res.data;
  }
